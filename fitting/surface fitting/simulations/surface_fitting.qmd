...
```{r}
pdf(file=file.path("figures/surface_fitting","surface_truth.pdf"), width=12, height=4)
par(mfrow=c(1,3))
par(mar=c(0,1,0,1))
...
pdf(file = file.path("figures/surface_fitting","surface_fitting.pdf"), width = 16, height = 9)
par(mfrow=c(3,8))
par(mar=c(0,0,0,0))
for(i in c(1:3)) {
  f = fs[[i]]; noise = noises[i]
  ...
  mebars_2 = mebars(x_train, y_obs, gamma = 1.0, times = c(2,2))
...
m_test = 250
# x_test = cbind(runif(m_test,0,1), runif(m_test,0,1))
xmin = apply(x_train, 2, min)
xmax = apply(x_train, 2, max)
x_test = cbind(runif(m_test, xmin[1], xmax[1]), runif(m_test, xmin[2], xmax[2]))
