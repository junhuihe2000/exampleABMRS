...
# plot counts
p_1 = ggplot(mapping=aes(x=counts)) + geom_bar(aes(y=after_stat(prop)), width = 0.6) + ylab("Proportion") + xlab("Number of Knots") + geom_vline(aes(xintercept=mean(counts)), color="red", linetype="dashed") + scale_x_continuous(breaks=1:6) + theme_classic()
# plot nodes
p_2 = ggplot(mapping=aes(x=nodes)) + geom_density(adjust=1) + ylab("Density") + xlab("Age in Years") + theme_classic()
...
# plot
p_fit = p_data + geom_line(aes(age, y_hat), color = 'red') + geom_vline(aes(xintercept=node[1]), linetype="solid") + geom_vline(aes(xintercept=node[2]), linetype="solid") +
