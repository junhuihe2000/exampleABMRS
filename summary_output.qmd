...
# Paths to saved arrays from the Slurm runs
out_dir <- "output/surface_fitting_nugget"
table_dir <- "tables/surface_fitting_nugget"
if(!dir.exists(table_dir)) {
    dir.create(table_dir, recursive = TRUE)
}
...
# Paths to saved arrays from the Slurm runs
out_dir <- "output/surface_fitting_appendix_nugget"
table_dir <- "tables/surface_fitting_appendix_nugget"
if(!dir.exists(table_dir)) {
    dir.create(table_dir, recursive = TRUE)
}
mse_files <- list.files(out_dir, pattern = "^surface_mse_raw_job-\\d+\\.rds$", full.names = TRUE)
